...
Sin embargo, se pierde un requisito crucial: el hecho de que `Clock` configure un temporizador y actualice la interfaz de usuario cada segundo debe ser un detealle de implementación de ``Clock`.

Idealmente, queremos escribir esto una ves y que `Clock` se actualice a sí mismo:
...
En aplicaciones con muchos componentes, es muy importante liberar resursos tomados por los componentes cuando se destruyen.
...
React puede agrupar varias invocaciones a `setState()` en una sola actulización para mejorar el rendimiento.

Debido a que `this.props` y `this.state` pueden actulizarse de forma asíncronica, no debes confiar en sus valores para calcular el siguiente estado.
...
## Las actulizaciones de estado se fusionan

Cuando invocas a `setState`, React combina el objeto que proporcionsate con el estado actual.
...
Ni los componentes padres o hijos pueden saber si un determinado componente tiene o no tiene estado y no les debería importar si se define como una función o un a clase.
...
A esto comúnmente se le llama flujo de datos «descendente» «unidireccional». Cualquier estado siempre es propiedad de algún componente específico, y cualquier dato o interfaz de usuario derivados de ese estado solo pueden afectar los componentes «debajo» de ellos en el árbol.
